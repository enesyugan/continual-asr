...
def concat_prompt(text, prompt, bos_token, eos_token):
    target = text.strip()
    full_text = prompt + bos_token + target + eos_token

    return full_text


class TEDLIUMDataCollatorForQwen2:
    ...
    def __call__(self, batch):
        ...
        # bos_token, eos_token = self.processor.bos_token, self.processor.eos_token
        ...
        text_list = [concat_prompt(example["text"], prompt, self.bos_token, self.eos_token) for example in batch]

        # Call the processor — it handles feature extraction, tokenization, padding
        ...
        # the qwen2audio model automatically handles audio insertion, audio token location

        return inputs


# TODO: add data collator for 6 datasets. Note we need torchaudio to load the data
