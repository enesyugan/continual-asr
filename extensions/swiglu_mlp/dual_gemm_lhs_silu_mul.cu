...
std::tuple<at::Tensor, at::Tensor, at::Tensor> dual_gemm_lhs_silu_and_mul_cutlass(
    const at::Tensor& x,
    const at::Tensor& w0,
    const at::Tensor& w1) {
  TORCH_CHECK(x.dim() == 2);
  ...
  using EpilogueOutputOp2 = EpilogueLHSSiLUAndMul<
      ElementOutput,
      128 / cutlass::sizeof_bits<ElementOutput>::value,
//       ActivationFn,
      ElementOutput,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> dual_gemm_lhs_silu_and_mul_(
  ...
  #define FWD_PARAMS x, w0, w1
  if (x.scalar_type() == at::ScalarType::Half) {
    return dual_gemm_lhs_silu_and_mul_cutlass<cutlass::half_t>(
        FWD_PARAMS);
  } else {
    TORCH_CHECK(
        x.scalar_type() == at::ScalarType::BFloat16, "Only supports bf16/f16");
    return dual_gemm_lhs_silu_and_mul_cutlass<cutlass::bfloat16_t>(
        FWD_PARAMS);
  }
}
