...
__global__ void spmm_kernel(const int64_t *rowptr_data,
  ...
  // rowptr_data: CSR row pointer (M+1 entries)
  // col_data: CSR col indices (nnz entries)
  // value_data: CSR non-zero values (nnz entries; only used if HAS_VALUE)
  // mat_data: Input dense matrix, shape [B, N, K] (row-major flattened)
  // out_data: Output, shape [B, M, K] (row-major flattened)
  // arg_out_data: (for reductions with argmax/argmin; may be unused)
  // B: batch size
  // M: number of sparse rows
  // N: number of sparse columns (and dense matrix rows)
  // K: number of output columns (dense matrix columns)

  // Each warp is responsible for a single output row (across batches) and a contiguous tile of 32 output columns in K.
  // Each thread in a warp handles one output column.

  // We ignore blockIdx.y here, because threads
  // across `blockIdx.y` are treated equally.
  int thread_idx = blockDim.x * blockIdx.x + threadIdx.x;

  // note: 32 threads per warp is a constant since Fermi to Blackwell
  ...
  if (batch_idx < B) {
    // Get the range of nonzero elements for this sparse row:
    int row_start = __ldg(rowptr_data + (row % M));
    int row_end = __ldg(rowptr_data + (row % M) + 1);

    // each thread in the warp starts with different offset
...
//   AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::Half, mat.scalar_type(), "_", [&] {
  AT_DISPATCH_FLOATING_TYPES_AND2(
    at::ScalarType::Half, at::ScalarType::BFloat16, mat.scalar_type(), "_", [&] {
