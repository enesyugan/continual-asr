...
def load_model_and_decode(rank, dataset_split, model_path, lora_path, tokenizer_path,
    ...
    if lora_path is not None and len(lora_path) > 0:
        ...
        processor = WhisperProcessor.from_pretrained(upstream_model)
        ...
        with torch.no_grad(), torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for idx, batch_ in tqdm(enumerate(data_loader), total=len(data_loader), position=rank,
                ...
                output_tokens = model.generate(inputs=batch["input_features"], forced_decoder_ids=forced_decoder_ids)
...
if __name__ == "__main__":
    ...
    test_dataset = load_asr_dataset(test_path, language=["bn", "en"])
    print(test_dataset)
